...
public class SessionService implements SessionUseCase {
    ...
    public void terminateSession(String username, String sessionId) {
        ...
        SessionMetadata sessionMetadata = sessionRepository.findBySessionId(sessionId).orElseThrow(
                () -> new NullPointerException("Session " + sessionId + " not found")
        );

        jwtTokenUseCase.blacklistJti(sessionMetadata.getRefreshJti(), "Session is terminated", (sessionMetadata.getExpiresAt() + 1000L) - System.currentTimeMillis());
