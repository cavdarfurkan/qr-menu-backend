...
public class SessionAdapter implements SessionRepositoryPort {
     ...
     * <p>
     * <ul>
     * <li>Key Type: {@code Set}</li>
     * <li>Member: {@code sessionId}</li>
     * </ul>
     *
     * @since 1.0.0
     */
    ...
    /**
     * <ul>
     * <li> Key Type: {@code Hash}</li>
     * <li> Field (Hash Key): {@code sessionId}</li>
     * <li> Value: {@link SessionMetadata} as JSON</li>
     * </ul>
     *
     * @since 1.0.0
     */
    ...
    public void saveSession(String username, SessionMetadata sessionMetadata) {
        ...
        redisTemplate.<String, SessionMetadata>opsForHash().put(ALL_SESSIONS, sessionMetadata.getSessionId(), sessionMetadata);
    }
    ...
    public Optional<SessionMetadata> findBySessionId(String sessionId) {
        SessionMetadata sessionMetadata = redisTemplate.<String, SessionMetadata>opsForHash().get(ALL_SESSIONS, sessionId);
    ...
    public boolean isSessionExpired(String sessionId) {
        SessionMetadata session = redisTemplate.<String, SessionMetadata>opsForHash().get(ALL_SESSIONS, sessionId);
        if (session == null) {
            throw new NullPointerException("sessionId " + sessionId + " not found");
        }
    ...
    public boolean isUserOwnsSession(String username, String sessionId) {
        String userSessionsKey = String.format(USER_SESSIONS, username);
        return Boolean.TRUE.equals(stringRedisTemplate.opsForSet().isMember(userSessionsKey, sessionId));
    }
}