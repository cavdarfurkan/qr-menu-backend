...
public class SessionControllerV1 {
    ...
    @GetMapping("/active")
    public ResponseEntity<ApiResponse<ActiveSessionsResponseDto>> activeSessions(@AuthenticationPrincipal UserDetails userDetails) {
        // TODO: If active sessions are too much (max 10), paginate the results
        List<SessionMetadata> sessionMetadataList = sessionUseCase.getActiveSessions(userDetails.getUsername());
    ...
    public ResponseEntity<ApiResponse<Void>> terminateSession(@Valid @RequestBody TerminateRequestDto terminateRequestDto, @AuthenticationPrincipal UserDetails userDetails) {
        try {
            sessionUseCase.terminateSession(userDetails.getUsername(), terminateRequestDto.getSessionId());
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ApiResponse.error("An error occurred during terminating session"));
        }
    }

    @PostMapping("/terminate-others")
    public ResponseEntity<ApiResponse<Void>> terminateAllOtherSessions(HttpServletRequest request, @AuthenticationPrincipal UserDetails userDetails) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ApiResponse.error("An error occurred during terminating sessions"));
        }
    }
