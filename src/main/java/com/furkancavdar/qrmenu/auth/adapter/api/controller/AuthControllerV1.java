...
public class AuthControllerV1 {
    ...
    @PostMapping("/login")
    public ResponseEntity<ApiResponse<LoginResponseDto>> loginUser(@Valid @RequestBody LoginRequestDto loginRequestDto, HttpServletRequest request, HttpServletResponse response) {
        ...
        Optional<LoginResultDto> optionalLoginResult = authenticationUseCase.login(
        ...
        LoginResponseDto loginResponseDto = LoginResponseMapper.fromLoginResultDto(optionalLoginResult.get());
        String refreshTokenCookie = buildRefreshTokenCookie(optionalLoginResult.get().getJwtTokenPair().getRefreshToken());
...
//    }
    ...
    @PostMapping("/refresh")
    public ResponseEntity<ApiResponse<RefreshResponseDto>> refreshToken(HttpServletRequest request, HttpServletResponse response) {
        log.info("auth controller v1: refresh");
        Cookie[] cookies = request.getCookies();
        if (cookies == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.error("Refresh token cookie is missing"));
        }
        ...
        Optional<RefreshResultDto> optionalRefreshResultDto = authenticationUseCase.refreshToken(refreshToken);
        ...
        RefreshResponseDto refreshResponseDto = RefreshResponseMapper.fromRefreshResultDto(optionalRefreshResultDto.get());
        String refreshTokenCookie = buildRefreshTokenCookie(optionalRefreshResultDto.get().getJwtTokenPair().getRefreshToken());
