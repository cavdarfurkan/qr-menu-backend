...
public class ThemeControllerV1 {
    ...
    @GetMapping("/{themeId}")
    public ResponseEntity<ApiResponse<ThemeManifestResponseDto>> getThemeManifest(@PathVariable String themeId) {
        try {
            ThemeManifestResultDto manifestResult = themeRegisterUseCase.getManifest(Long.valueOf(themeId));
            ThemeManifestResponseDto response = ThemeManifestResponseMapper.fromThemeManifestResultDto(manifestResult);
    ...
    @GetMapping("/test")
    public ResponseEntity<ApiResponse<UserDetails>> test(@AuthenticationPrincipal UserDetails userDetails) {
    ...
    public ResponseEntity<ApiResponse<String>> registerTheme(
        ...
        if (!validateZipUpload.isValid()) {
            return ResponseEntity
                    .badRequest()
                    .body(ApiResponse.error(validateZipUpload.message()));
        }
        ...
        if (imageFile != null && !imageFile.isEmpty()) {
            ...
            if (!validateImageUpload.isValid()) {
                return ResponseEntity
                        .badRequest()
                        .body(ApiResponse.error(validateImageUpload.message()));
            }
        }
        ...
        return ResponseEntity
                .ok(ApiResponse.success("Theme registered successfully"));
    }
    ...
    public ResponseEntity<ApiResponse<String>> unregisterTheme(
        ...
        try {
            ...
            themeRegisterUseCase.unregisterTheme(unregisterThemeRequestDto.getThemeId(), userDetails.getUsername(), isAdmin);
        } catch (Exception e) {
    ...
    private ValidateResponse validateZipUpload(MultipartFile zipFile) {
        ...
        if (zipFile.getOriginalFilename() != null && !zipFile.getOriginalFilename().toLowerCase().endsWith(".zip")) {
    ...
    private ValidateResponse validateImageUpload(MultipartFile imageFile) {
        if (imageFile.getOriginalFilename() != null &&
            ...
            String suffixes = Arrays.stream(ImageSuffix.values())
                    .map(ImageSuffix::getSuffix)
