...
public class MenuContentControllerV1 {
  ...
  public ResponseEntity<ApiResponse<HydratedItemResponseDto>> updateContent(
    ...
    HydratedItemDto updatedContent =
    ...
    return ResponseEntity.ok(
        ApiResponse.success(HydratedItemResponseMapper.fromHydratedItemDto(updatedContent)));
  }
  ...
  public ResponseEntity<ApiResponse<List<HydratedItemResponseDto>>> getCollectionContent(
      ...
      @AuthenticationPrincipal UserDetails userDetails,
      @Valid @PathVariable @NotBlank String collection) {
    List<HydratedItemDto> content =
        menuContentUseCase.getCollectionContent(userDetails.getUsername(), menuId, collection);
    return ResponseEntity.ok(
        ApiResponse.success(
      ...
      @Valid
          ...
          @Pattern(
              regexp =
                  "^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$")
          String itemId) {
    HydratedItemDto content =
        menuContentUseCase.getContent(
            userDetails.getUsername(), menuId, collection, UUID.fromString(itemId));
    return ResponseEntity.ok(
        ApiResponse.success(HydratedItemResponseMapper.fromHydratedItemDto(content)));
  }
  ...
  public ResponseEntity<ApiResponse<Void>> deleteContent(
    ...
    return ResponseEntity.ok(ApiResponse.success("Menu content deleted successfully"));
  }
  ...
  public ResponseEntity<ApiResponse<Void>> deleteContentBulk(
    ...
    log.info(
        "MenuContentControllerV1:deleteContentBulk {} menu content items deleted successfully",
        requestDto.getItemIds().size());
    return ResponseEntity.ok(
        ApiResponse.success(
            String.format(
                "%d menu content items deleted successfully", requestDto.getItemIds().size())));
  }
}
