...
public class MenuContentService implements MenuContentUseCase {
  ...
  @Override
  @Transactional
  public HydratedItemDto createContent(
    ...
    if (relations != null && !relations.isEmpty()) {
      for (Map.Entry<String, List<UUID>> entry : relations.entrySet()) {
        ...
        for (UUID targetId : targets) {
          MenuContentItem targetItem =
              menuContentRepository
                  .findById(targetId)
                  .orElseThrow(
                      () ->
                          new ResourceNotFoundException(
                              "Target content with id %s not found".formatted(targetId)));
  ...
  @Override
  @Transactional
  public HydratedItemDto updateContent(
      String currentUsername,
      Long menuId,
      String collection,
      UUID itemId,
      JsonNode newContent,
      Map<String, List<UUID>> newRelations) {
    ValidateResult validateResult = validateUserAndMenu(currentUsername, menuId, collection);
    Menu menu = validateResult.menu;
    ...
    MenuContentItem menuContentItem =
        menuContentRepository
            .findById(itemId)
            .orElseThrow(
                () -> new ResourceNotFoundException("Item with id %s not found".formatted(itemId)));
  ...
  @Override
  @Transactional
  public List<HydratedItemDto> getCollectionContent(
      String currentUsername, Long menuId, String collection) {
    validateUserAndMenu(currentUsername, menuId, collection);

    List<MenuContentItem> menuContentItem =
        menuContentRepository.findByMenuIdAndCollectionName(menuId, collection);
    if (menuContentItem.isEmpty()) {
      throw new ResourceNotFoundException(
          "Content for collection %s not found".formatted(collection));
    }

    return menuContentItem.stream().map(MenuContentItem::getId).map(this::hydrate).toList();
  }

  @Override
  @Transactional
  public HydratedItemDto getContent(
      String currentUsername, Long menuId, String collection, UUID itemId) {
    ValidateResult validateResult = validateUserAndMenu(currentUsername, menuId, collection);

    MenuContentItem menuContentItem =
        menuContentRepository
            .findByMenuIdAndCollectionNameAndId(menuId, collection, itemId)
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        "Content with id %s not found in collection %s"
                            .formatted(itemId, collection)));
  ...
  public void deleteContent(String currentUsername, Long menuId, String collection, UUID itemId) {
    ...
    if (menuContentRepository.existsByTargetItemId(itemId)) {
      List<MenuContentRelation> referencingRelations =
          menuContentRepository.findByTargetItemId(itemId);
      Set<String> referencingSources =
          referencingRelations.stream()
              .map(
                  rel ->
                      rel.getSourceItem().getCollectionName() + ":" + rel.getSourceItem().getId())
  ...
  public HydratedItemDto hydrate(UUID itemId) {
    ...
    Set<UUID> allTargetIds =
        relations.stream().map(r -> r.getTargetItem().getId()).collect(Collectors.toSet());
    Map<UUID, JsonNode> targets =
        menuContentRepository.findAllByIdIn(allTargetIds).stream()
    ...
    for (String field : byField.keySet()) {
      List<MenuContentRelation> ordered =
          menuContentRepository.findBySourceItemIdAndFieldNameOrderByPositionAsc(itemId, field);
  ...
  private void validateTargetsBelongToMenu(Long menuId, Set<UUID> targetIds) {
    ...
    boolean badMenu =
        found.stream().anyMatch(item -> !Objects.equals(item.getMenu().getId(), menuId));
    if (badMenu) {
      throw new IllegalArgumentException("Target items must belong to the same menu");
    }
  }

  private ValidateResult validateUserAndMenu(String currentUsername, Long menuId, String collection)
    ...
    if (!menu.getSelectedTheme().getThemeSchemas().containsKey(collection)) {
      throw new ResourceNotFoundException(
          "Schema for collection %s not found".formatted(collection));
    }
