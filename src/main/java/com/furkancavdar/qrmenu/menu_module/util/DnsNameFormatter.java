...
//        label = label.replaceAll("-{2,}", "-");
        ...
        if (label.isEmpty()) {
            throw new IllegalArgumentException("Cannot convert input to a valid DNS label: \"" + input + "\"");
        }
    ...
    /**
     * Convert a full domain name (possibly with dots) into an RFC-1123 compliant name.
     * Each label between dots is cleaned independently.
     */
