...
public class MenuContentControllerTest {
  ...
  @Autowired private MenuContentRepositoryPort menuContentRepository;
  ...
  public void create_content_with_relation() throws Exception {
    ...
    mvc.perform(
            post("/api/v1/menu/{menuId}/content", menu.getId())
  ...
  public void update_without_relation() throws Exception {
    ...
    var createBody =
        Map.of("collection", "category", "content", Map.of("name", "Coffee", "slug", "coffee"));
    ...
    var contentId =
        mapper.readTree(res.getResponse().getContentAsByteArray()).get("data").get("id").asText();

    var updateBody = Map.of("new_content", Map.of("name", "Coffee", "slug", "updated-coffee"));

    mvc.perform(
            put(
                    "/api/v1/menu/{menuId}/content/{collection}/{itemId}",
                    menu.getId(),
                    "category",
                    contentId)
                .contentType(MediaType.APPLICATION_JSON.toString())
  ...
  public void update_with_relation() throws Exception {
    ...
    var productId =
        mapper
            .readTree(createRes.getResponse().getContentAsByteArray())
            .get("data")
            .get("id")
            .asText();
  ...
  public void get_hydrated_collection_content() throws Exception {
    ...
    mvc.perform(
            get("/api/v1/menu/{menuId}/content/{collection}", menu.getId(), "product")
                .header("Authorization", "Bearer " + accessToken))
        .andExpect(status().isOk())
        ...
        .andExpect(
            jsonPath("$.data[?(@.data.name == 'Double Espresso Collection Test')].data.price")
                .value(47))
        .andExpect(
            jsonPath(
                    "$.data[?(@.data.name == 'Double Espresso Collection Test')].resolved.category[0].slug")
                .value("hot-drinks"))
        // Verify second product with hydrated relation
        .andExpect(
            jsonPath("$.data[?(@.data.name == 'Iced Latte Collection Test')].data.price").value(57))
        .andExpect(
            jsonPath(
                    "$.data[?(@.data.name == 'Iced Latte Collection Test')].resolved.category[0].slug")
                .value("cold-drinks"))
        // Verify third product with hydrated relation
        .andExpect(
            jsonPath("$.data[?(@.data.name == 'Tiramisu Collection Test')].data.price").value(67))
        .andExpect(
            jsonPath(
                    "$.data[?(@.data.name == 'Tiramisu Collection Test')].resolved.category[0].slug")
                .value("desserts"));
  }
  ...
  public void get_hydrated_content() throws Exception {
    ...
    var categoryId =
        mapper
            .readTree(categoryRes.getResponse().getContentAsByteArray())
            .get("data")
            .get("id")
            .asText();
    ...
    var productId =
        mapper
            .readTree(productRes.getResponse().getContentAsByteArray())
            .get("data")
            .get("id")
            .asText();
  ...
  public void delete_content_should_remove_item() throws Exception {
    ...
    // Create a product
    var productBody =
        Map.of("collection", "product", "content", Map.of("name", "Espresso", "price", 45));
  ...
  public void delete_content_bulk_should_remove_multiple_items() throws Exception {
    ...
    var product1Body =
        Map.of("collection", "product", "content", Map.of("name", "Coffee", "price", 40));
    ...
    var product1Id =
        mapper
            .readTree(product1Res.getResponse().getContentAsByteArray())
            .get("data")
            .get("id")
            .asText();

    var product2Body =
        Map.of("collection", "product", "content", Map.of("name", "Tea", "price", 30));
    ...
    var product2Id =
        mapper
            .readTree(product2Res.getResponse().getContentAsByteArray())
            .get("data")
            .get("id")
            .asText();

    var product3Body =
        Map.of("collection", "product", "content", Map.of("name", "Juice", "price", 50));
    ...
    var product3Id =
        mapper
            .readTree(product3Res.getResponse().getContentAsByteArray())
            .get("data")
            .get("id")
            .asText();

    // Bulk delete product1 and product2
    var deleteBody = Map.of("item_ids", List.of(product1Id, product2Id));

    mvc.perform(
            delete("/api/v1/menu/{menuId}/content/{collection}", menu.getId(), "product")
                .contentType(MediaType.APPLICATION_JSON.toString())
  ...
  public void delete_content_bulk_with_relations_should_cascade() throws Exception {
    ...
    // Create categories
    var category1Body =
        Map.of("collection", "category", "content", Map.of("name", "Hot", "slug", "hot"));
    ...
    var category1Id =
        mapper
            .readTree(category1Res.getResponse().getContentAsByteArray())
            .get("data")
            .get("id")
            .asText();

    var category2Body =
        Map.of("collection", "category", "content", Map.of("name", "Cold", "slug", "cold"));
    ...
    var category2Id =
        mapper
            .readTree(category2Res.getResponse().getContentAsByteArray())
            .get("data")
            .get("id")
            .asText();
  ...
  @Test
  public void delete_content_bulk_should_return_not_found_for_non_existent_items()
      throws Exception {
    setup();
    ...
    String nonExistentId = java.util.UUID.randomUUID().toString();
    var deleteBody = Map.of("item_ids", List.of(productId, nonExistentId));
  ...
  public void delete_content_bulk_should_reject_more_than_100_items() throws Exception {
    ...
    var deleteBody = Map.of("item_ids", tooManyIds);
  ...
  private void authenticate() throws Exception {
    ...
    String username =
        mapper
            .readTree(registerRes.getResponse().getContentAsByteArray())
            .get("data")
            .get("username")
            .asText();
    user =
    ...
    // Login
    var loginBody = Map.of("username", user.getUsername(), "password", "123123123");
  ...
  private void setup() throws Exception {
    ...
    themeSchemas.put(
        "product",
        mapper.readTree(
            "{\"$ref\":\"#/definitions/product\",\"$schema\":\"http://json-schema.org/draft-07/schema#\",\"definitions\":{\"product\":{\"type\":\"object\",\"required\":[\"name\",\"price\"],\"properties\":{\"name\":{\"type\":\"string\"},\"price\":{\"type\":\"number\"}},\"additionalProperties\":false}}}"));
    themeSchemas.put(
        "category",
        mapper.readTree(
            "{\"$ref\":\"#/definitions/category\",\"$schema\":\"http://json-schema.org/draft-07/schema#\",\"definitions\":{\"category\":{\"type\":\"object\",\"required\":[\"slug\",\"name\"],\"properties\":{\"name\":{\"type\":\"string\"},\"slug\":{\"type\":\"string\"}},\"additionalProperties\":false}}}"));
    Map<String, JsonNode> uiSchemas = new HashMap<>();

    // Create a theme
    theme =
        themeRepositoryPort.save(
            new Theme(
                user,
                "thumbnail_url",
                "location_url",
